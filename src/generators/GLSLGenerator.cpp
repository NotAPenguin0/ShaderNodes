...
struct generator {
    ...
    std::string get_pin_name(node_pin_id pid) {
    ...
    void operator()(ShaderNode const& node) {
        if (node.type == nodes::input_value) {
            ...
            std::string pin_name = get_pin_name(outputs[0]);
        ...
        if (node.type == nodes::multiply) {
            ...
            assert(outputs.size() == 1);
            std::string input_name_a = get_pin_name(inputs[0]);
            std::string input_name_b = get_pin_name(inputs[1]);
            std::string output_name = get_pin_name(outputs[0]);
        ...
        if (node.type == nodes::output_value) {
            ...
            std::string input_name = get_pin_name(inputs[0]);
